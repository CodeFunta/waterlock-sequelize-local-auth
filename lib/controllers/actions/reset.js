...
function issuePasswordReset(req, res, sails, params){
  sails.models.auth.findOne({ where:{ id:req.session.resetToken.owner} }).populate('resetToken').exec(function(err, auth){
    if(err){
      return res.serverError(err);
    }
    ...
    sails.models.auth.update({id:auth.id},{resetToken: null, password: params.password}).exec(function(err, auth){
      ...
      // destroy the token
      sails.models.resettoken.destroy({where:{id:tokenId}}).then(function(){
