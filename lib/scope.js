...
function generateScope(scopeKey, engine) {
    return {
        ...
        registerUserAuthObject: async (attributes, req, cb, mapUserCriteria,userdefaults) =>{
            var self = this;
            var attr = {
            ...
            return await self.engine.findOrCreateAuth(criteria, attr, mapUserCriteria,userdefaults);
        ...
        getUserAuthObject: async (attributes, req) => {
            ...
            if (authConfig.createOnNotFound) {
                return await this.engine.findOrCreateAuth(criteria, attr);
            } else {
                return await this.engine.findAuth(criteria);
            }
        }
    };
}
